...
%%%%%== Subsubsec: Bit I ==%%%%%
\subsubsection{Basic Bit Operations: True or False}
...
\vspace{16pt}
\noindent\textbf{XOR Operation}: This is commonly used in cryptography for things like one-time pads.
...
\vspace{16pt}
\newpage
\noindent\textbf{Applying Functions to Sequences:} In Cryptol, you can apply functions to entire sequences, which is useful for cryptographic transformations.
\begin{lstlisting}[style=cryptol]
...
We define a record with braces `\texttt{$\{\}$}' and specify the names and types of its fields.
\end{tcolorbox}
\begin{lstlisting}[style=cryptol]
type MyRecord = {
	field1 : Bit,
